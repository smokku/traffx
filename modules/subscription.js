  ...
  return function subscription (stanza, res, next) {
    if (shouldHandle(stanza)) {
      ...
      if (stanza.type === 'subscribe') {
        ...
        Roster.one(stanza.to, stanza.from)
          .then(item => {
            if (item && item.from) {
              ...
              if (item && item.approved) {
                ...
                res.type = 'subscribed'
                res.send()
                Roster.set(stanza.to, stanza.from, { from: true })
                  .then(item => {
                ...
                Roster.set(stanza.to, stanza.from, {
                  in: stanza.toString()
                }).catch(next)
          ...
          .then(item => {
            if (item) {
              ...
              if (!item.to && item.ask) {
                ...
                Roster.set(stanza.to, stanza.from, { to: true, ask: null })
                  .then(item => {
                    router.route(stanza.to, rosterPush(stanza.to, item))
                  })
                  .catch(next)
          ...
          .then(item => {
            if (item) {
              ...
              if (item.from) {
                ...
                router.route(stanza.to, stanza)
                // 2. Initiate a roster push
                Roster.set(stanza.to, stanza.from, { from: false })
                  .then(item => {
                    router.route(stanza.to, rosterPush(stanza.to, item))
                  })
                  .catch(next)
          ...
          .then(item => {
            if (item) {
              ...
              if (item.to || item.ask) {
                ...
                router.route(stanza.to, stanza)
                // 2. Initiate a roster push
                Roster.set(stanza.to, stanza.from, { to: false, ask: null })
                  .then(item => {
                    router.route(stanza.to, rosterPush(stanza.to, item))
                  })
                  .catch(next)
...
module.exports.outbound = function (router) {
  ...
  return function subscription (stanza, next) {
    if (shouldHandle(stanza)) {
      ...
      Roster.one(stanza.from, stanza.to)
        .then(item => {
          if (
            ...
            if (stanza.type === 'subscribed') {
              ...
              if (!item || !item.in) {
                ...
                Roster.set(stanza.from, stanza.to, { approved: true })
                  .then(item => {
            ...
            Roster.set(stanza.from, stanza.to, change)
              .then(item => {
                router.route(stanza.from, rosterPush(stanza.from, item))
              })
              .catch(next)
